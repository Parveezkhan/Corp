...
const Dropdowns = () => {
  ...
  const handleCollectConfig = (e) => {
    e.preventDefault();
    setConfigurations({ ...configurations, [e.target.name]: e.target.value });
  };
  const handle = (e) => {
    e.preventDefault();
    console.log(e);
  ...
  const handleData = (event) => {
    ...
    if (Number(event.target.ram_cost.value)) {
      setRam_cost(Number(event.target.ram_cost.value));
    }
    if (Number(event.target.cpu_cost.value)) {
      setCpu_cost(Number(event.target.cpu_cost.value));
    }
    if (Number(event.target.ssd_cost.value)) {
      setSsd_cost(Number(event.target.ssd_cost.value));
    }
    if (Number(event.target.public_ip_cost.value)) {
      setPublic_ip_cost(Number(event.target.public_ip_cost.value));
    }
  ...
  const calculateCost = () => {
    ...
    let server_day_charges = Math.ceil(
      Number(configurations.ram * ram_cost) +
        Number(configurations.cpu) * cpu_cost +
        Number(configurations.storage) * ssd_cost
    );
    {
      configurations.day === "Half Day"
        ? (server_day_charges = server_day_charges - server_day_charges * 0.3)
        : (server_day_charges = server_day_charges);
    }
    ...
    //server_cost_batch
    let server_cost_per_batch = Math.ceil(
      server_day_charges *
        (Number(configurations.no_of_servers) *
          Number(configurations.no_of_days))
    );
          ...
          <div className="row p-4 h6">
            <div className="col-sm-12 col-md-6 col-lg-6 col-12 d-flex justify-content-between flex-row flex-wrap border border-4 border-info p-4 rounded my-2">
              <div className="row w-100 ">
                ...
                >
                  ...
                  <div className="col-sm-12 col-md-6 col-lg-6 col-12 w-40 px-2  ">
                    ...
                    <div className="dropdown">
                      ...
                      >
                        ...
                        <option value="Infrastructure_Labs">
                          {" "}
                          Infrastructure Labs
                        </option>
                      </select>
                  ...
                  <div className="total cost text-center w-100">
                    <h2 className=" my-2">Total cost:{total_cost}</h2>
            ...
            <div className="col-sm-12 col-md-6 col-lg-6 col-12 p-4 d-flex justify-content-center ">
              <div className="input_data w-50 p-3  h6 border border-4 rounded border-info h-100">
                ...
                <h2 className="text-center my-2">Total cost:{total_cost}</h2>
              </div>
            </div>
          </div>
        </div>
